...
generate_thumbnail() {
    ...
    # GIFs: use the first frame; videos are skipped
    case "${abs_path,,}" in
        *.mp4|*.webm|*.mkv|*.avi|*.mov)
            return
            ;;
    esac
    local magick_input="$abs_path"
    case "${abs_path,,}" in
        *.gif) magick_input="${abs_path}[0]" ;;
    esac
    local encoded_path
    ...
    magick "$magick_input" -resize "${THUMBNAIL_SIZE}x${THUMBNAIL_SIZE}" "$out"
}
